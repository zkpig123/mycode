...
int main (void)
{
	...
	int *orig_cards = NULL;
	size_t card_num;
	int(*funcs[6])(int[], size_t, int);
	funcs[0] = merge_sort_non_guard;
	funcs[1] = merge_sort_guard;
	...
	for (card_num = 1, last_card_num = 0; card_num < MAX_CARD_NUM; ){
		...
		printf("card_num:%zd\n", card_num);
		fflush(stdout);
		various_sort_cards (cards, orig_cards, card_num, funcs);
...
{
	if (cur_func == 0) printf("success: merge sort non guard, time used:%f", elapsed);
...
int get_child_runtime (double *value)
{
	struct tms buf;
	if (times(&buf) == (clock_t)-1) return 1;
	*value = 1.0 * (buf.tms_cutime + buf.tms_cstime) / clock_ticks;
	return 0;
}
	...
	if (pid == 0){
		...
		if (ret) print_sort_func_return_err(cur_func, ret);
			...
			if (check_sort_result (cards, orig_cards, card_num) == 0){
				...
				print_success_msg(cur_func, elapsed);
...
void various_sort_cards (int cards[], int orig_cards[], size_t card_num, int(*funcs[6])(int[], size_t, int))
{
	for (int cur_func = 0; cur_func < 5; cur_func++){
