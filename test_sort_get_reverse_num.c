...
int main (void)
{
	...
	int *orig_cards = NULL;
	size_t card_num;
	int(*funcs[5])(int[], size_t, int, size_t*);
...
void various_sort_cards (int cards[], int orig_cards[], size_t card_num, int(*funcs[])(int[], size_t, int, size_t*))
{
...
int init_random_number (void)
{
	...
	rand = tv.tv_sec * tv.tv_usec + tv.tv_sec + tv.tv_usec;
	srand(rand);
	return 0;
}
